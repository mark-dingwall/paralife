...
**Token set is closed for Phase 17.** Future phases extend `17-ADMISSION.md` with new entries.

Reserved but not emitted this phase: ingress-flood token (D-09 chose counter-only, no kill).

---

## §2 Wire Shape Delta vs `15-SCHEMA.md`

See `15-SCHEMA.md` §8.3 for the canonical frame grammar; this document only defines the new resume-token slot and the closed token vocabulary in §1.
