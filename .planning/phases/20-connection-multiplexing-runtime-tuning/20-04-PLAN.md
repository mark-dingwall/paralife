...
must_haves:
  truths:
    ...
    - "Each recipe cites a specific JFR baseline file from Plan 1c's profiles/ directory by exact filename including the `62c1b44` SHA suffix (the post-20-01c F6 re-anchor; the original `c22e487` baseline was superseded — D-19 in 20-CONTEXT.md still nominally names `c22e487` and is a known doc-drift cleanup, to be reconciled in Plan 6)."
  ...
  key_links:
    - from: "20-RUNTIME.md §3.1, §3.2, §3.3"
      to: "profiles/jfr-{100,500,1000}bots-baseline-62c1b44.jfr"
      via: "filename citation in recipe text"
      pattern: "jfr-[0-9]+bots-baseline-62c1b44\\.jfr"
...
Replace `_Plan X_` and "_Pending_" markers as those plans land. Task 4.2 fills §3.
  ...
  <acceptance_criteria>
    ...
    - `grep -q "62c1b44" 20-RUNTIME.md` exits 0 (D-19 anchor cited)
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <action>Edit 20-RUNTIME.md to fill §3 with three per-tier recipes (§3.1 100-bot, §3.2 500-bot, §3.3 1000-bot) using the templates in <how-to-verify>; each recipe cites the corresponding baseline JFR by exact filename including the 62c1b44 SHA suffix; each recipe explicitly marks GC choice as `Pending — JFR-driven` per D-13. Then a human runs each recipe to confirm copy-pasteability (server boots, harness connects).</action>
  <read_first>
    ...
    - .planning/phases/20-connection-multiplexing-runtime-tuning/20-01c-SUMMARY.md (baseline JFR filenames at SHA `62c1b44`; A8 ZGC default-on outcome; F6 baseline re-anchor rationale superseding `c22e487`)
...
1. Copies the §3.X recipe verbatim into a shell (two terminals: server then harness).
2. Confirms the server prints `Started ParalifeApplication` within 30s.
3. Confirms the harness emits `LoadHarness starting — harness-id=... count=N ...` (SLF4J `INFO` from `LoadHarness.java:196`) and runs to its `--duration` without errors. The harness writes its final report JSON containing `peak_registered` if `--report-out` is supplied; otherwise the operator confirms by observing zero stack traces and clean process exit (rc=0).
4. Stops the server cleanly (SIGTERM).
5. Confirms a JFR file with the recipe-named filename appears in the server CWD.
...
- 20-RUNTIME.md §3 cites all three baseline JFR filenames from Plan 1c
- All three recipes carry an explicit `Pending — JFR-driven` marker for GC choice (D-13 honesty: no untraceable recommendations)
- Recipe yaml override blocks DO NOT include `queue-watermark-pct` as an actual yaml key (Pass-2 Concern #8). Manual check during human verification: scan each ```yaml fenced block in §3.2/§3.3 — no `queue-watermark-pct:` line (commented prose mentioning the key is intentional). The previously-proposed automated grep is omitted as both anchor-mismatch and comment-text false-positives make it unreliable; the human-verify gate is the check.
  </how-to-verify>
...
- Each recipe cites a baseline JFR filename matching the 62c1b44 anchor
...
- D-19 honoured: every JFR citation in §3 contains the 62c1b44 SHA suffix
