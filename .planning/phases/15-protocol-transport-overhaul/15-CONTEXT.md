...
**Gathered:** 2026-04-19
**Status:** Ready for planning (schema review complete — see `15-SCHEMA.md`)

> **⚠ Wire-format decisions below are superseded by `15-SCHEMA.md` (locked 2026-04-20).**
> Individual decisions tagged *[Superseded by 15-SCHEMA.md]* are retained for historical
> context and audit trail. Any conflict with `15-SCHEMA.md` resolves in favour of the schema.
> See `15-SCHEMA.md` §9 "Decision Reversal Table" for the full diff.
...
- **D-07:** *[Partially superseded by 15-SCHEMA.md §6.3 — `season=` slot removed; header is `T|<tickId>|<curX><curY>|<energy>/<maxEnergy>|<sensorRadius>`, tagged blocks use letter-prefix (no `=`).]* Framing is pipe-delimited, hybrid of positional + tagged:
...
- **D-09:** *[Superseded by 15-SCHEMA.md §6.3 — `<curX><curY>` absolute position now in every T header.]* Absolute world x,y **dropped** from self block. Zero-trust — pure relative vision. Post-MVP feature (return-to-spawn, spatial memory) may reintroduce as dedicated section if justified.
...
- **D-13:** *[Superseded by 15-SCHEMA.md §8.4 — event catalogue expanded beyond minimum-non-derivable set.]* Events are only included when **not derivable** from the frame otherwise. Formal redundancy rule:
...
- **D-14:** *[Superseded by 15-SCHEMA.md §8.4 — full event table: E/A/H/T/M/R/L/N/S/D with numpad/relative coords.]* Event catalogue (minimum-non-derivable set):
  ...
  | `CLEAVE` | — | Same |
  | `DSLV` | — | Same |
- **D-15:** *[Superseded by 15-SCHEMA.md §8.4 — E/R kept as events; T/M kept; LH migrated to event `vL` with rel/numpad coord; CNS/IF/CURE/STARVED_TICK remain derivable.]* Dropped events (derivable, not on wire):
...
- **D-16:** *[Superseded by 15-SCHEMA.md §8.3 — `f` prefix (not `b=`); effect codes I/F/A/M/S/U; `CR` reproduce cooldown dropped (client-derives); `X` phantom removed; FLEEING added with abs strike coord.]* Effects list (`b=` section) content. Entries separated by `;`. Format `<code>:<expiryTick>[,<args>]`:
...
- **D-17:** *[Superseded by 15-SCHEMA.md §8.3 — no cooldown codes on wire at all.]* Per-action-type cooldown codes (D-16 `CR`, future `CA`). Single generic `COOLDOWN:<action>` rejected for brevity + extensibility.

### Entity Status Bitmasks (Phase 14 D-38/D-39 carried forward, revised)
- **D-18:** *[Partially superseded by 15-SCHEMA.md §8.1.3 — same bit layout, but delivered via `envState` char in `s` entries gated by presence bitmask (not `:` sentinel).]* `cellStatus` byte — 6 bits used, 3 reserved for future. Vision-scoped OVERCROWDED (per Phase 14 D-40) kept — bots at vision edge legitimately miscount. D-40 mask-and-OR recomposition in `PerceptionBroadcaster.cellToView` preserved.
  | Bit | Flag | Source |
  |---|---|---|
  ...
  | 3–5 | reserved | — |
- **D-19:** *[Partially superseded by 15-SCHEMA.md §8.1.2 — same bit layout (STARVING/MUTATING/BUFFED), delivered via `entityState` char in `s` entries; omitted when value = 0.]* `entityStatus` byte revised — **3 bits used, 3 reserved.** STARVING removed (redundant vs energy X/Y on self + `Cell.FLAG_STARVING` global bit on cell flags). TOXIC removed (derivable from occupant-cell `TOXIN_PRESENT` bit — entity doesn't carry toxin after leaving cloud; splash-damage rule is protocol-implicit not entity-intrinsic).
  | Bit | Flag | Source |
  |---|---|---|
...
### Vision Section `c=`
- **D-21:** *[Partially superseded by 15-SCHEMA.md §8.1 — entry shape is now `<coord><presence><kindData?><entityState?><envState?>`; RLE is kind-only with numpad dir + base64 additional-count; env supplements via separate presence=2 entries.]* Sparse + RLE encoding. Only non-default (occupied, status-bearing, or RLE-compressible) cells emitted. Separator between tokens: `,`.
- **D-22:** *[Superseded by 15-SCHEMA.md §8.1 — kind codes are single-char C/M/S/D/N/T/0-5/F/R; composite roles are digits 0-5; nutrient is F; presence bitmask replaces status sentinel.]* In-vision cell token grammar:
  ```
...
- **D-23:** *[Superseded by 15-SCHEMA.md §8.4 — lightning migrated to events (`v<coord>L<X>`); no far-perception markers in `s` block; `L` ambiguity eliminated.]* Out-of-vision / far-perception markers allowed in `c=`. Token: `<coord><markerChar>` where `markerChar` signals a world-scale event whose coord can exceed sensor radius. Markers reserved for future: `L` lightning strike epicentre, others TBD. **Open:** marker namespace collides with role letters (L = lightning OR LOCOMOTOR) — disambiguation rule needed (parser knows role letters only appear after kind=6 subcode; far markers appear as bare `<coord><letter>` tokens). See D-50.

### Composite Tick Frame — `CT`
- **D-24:** *[Superseded by 15-SCHEMA.md §7 — 3-tier authority: full (solo/bonded/LOCO), authority-lite (FEED/ATT/REP, radius-1), passive (SENS/DEF, minimal T). Single `T` frame char for all tiers; no separate CT/MT.]* Composite members whose role grants decision authority receive full `CT` frame. Non-authority members receive minimal `MT` frame.
...
- **D-25:** *[Superseded by 15-SCHEMA.md §6.3/§8.5 — `p<pool>/<maxPool>` (no `=`, no `:<size>`); `g<coord><role>,...` roster covers all members on-change-only; fixed-width coord+role lets `:` separator drop.]* `CT` frame adds two tagged sections on top of `T`:
...
- **D-26:** *[Superseded by 15-SCHEMA.md §6.3.2 — passive members receive minimal `T|<tickId>|<curX><curY>|<energy>/<maxEnergy>[|v<event>,...]`; IRV replaces plurality for LOCOMOTOR vote.]* Non-authority members receive `MT|<tick>|<energy>/<max>[|ev=<events>]`. Alive-check + own events only, no vision, no effects, no pool state. Substantially smaller payload per tick.
...
- **D-28:** *[Carried forward, stricter: 15-SCHEMA.md §8.1 hides bonded-secondary type as well — `D`/`N`/`T` mark primary only.]* Neighbour occupant IDs dropped entirely from wire. Visible cells carry only `<coord><kindCode>[<subcodes>][<statusBlock>]`. Cross-tick neighbour tracking impossible from protocol alone — matches stateless-bot redesign goal. Composite coordination continues to work: each member knows its own composite via `t=` state-change + cached.
...
- **D-37:** *[Superseded by 15-SCHEMA.md §9 — no bulk rock map; rocks delivered per-cell in vision only (zero-trust). Visualiser / M005 may receive separately.]* Rock map delivery mechanism between server and client: **open (D-50)** — embed in `W` (Welcome) frame, or send as separate one-shot frame after Welcome. Same-payload-across-sessions property means it's the one place where precompress-once would genuinely save bytes on reconnect storms; listed in D-50 as a decision point.
...
- **D-45:** *[Superseded by 15-SCHEMA.md §5/§6 — frame inventory reduced to r/S/T/a/E. `W` (Welcome) and standalone `R` (Registered) collapse into `r`→`S` handshake.]* Big-bang rollout means auxiliary frames also go compact. Shape guidance (planner refines):
...
- **D-46:** *[Superseded by 15-SCHEMA.md §6.4/§8.6 — unified `a|<verb>[|<arg>]`. Verbs: M/E/A/R/V/L. V carries 3-char numpad rank string.]* Action frame (client→server) — shape follows tick grammar (pipe-delimited, verb + optional args). Exact syntax locked in formal schema review (D-50). Likely `A|<verb>|<dir?>|<ranks?>` where verbs are `M` move / `C` consume / `R` reproduce / `Z` rest, and ranks are the composite LOCOMOTOR STV preference list (concatenated direction chars).
...
- **D-48:** *[Resolved by 15-SCHEMA.md §2 — numpad `1`-`9` (single spec covers 8 dirs + `5`=self marker + rank strings).]* **Direction char encoding** — single-char encoding for compass directions used in RLE run suffixes, `ATT_IN` / `ATT_OUT` args, FLEEING from-coord convention, Action frame direction slot. Candidates: 8 alphabetic (`N E S W U V X Y` where U=NE, V=SE, X=SW, Y=NW) vs numeric `0-7`. **User-gated — surface during schema review.**
- **D-49:** *[Resolved by 15-SCHEMA.md §5 — client-derives via shared `SimulationConfig`; zero wire cost; `season=` slot removed entirely.]* **Season multiplier encoding** — how the sine-wave multiplier is packed into the `season=<season>,<multiplier>` section. Candidates: fixed-point decimal string `"1.05"`, base64 fixed-point integer, or omit from wire and re-derive client-side from season phase + tick. **User-gated — surface during schema review.**
- **D-50:** *[GATE SATISFIED 2026-04-20. See `15-SCHEMA.md` §9 for full resolution table. All 9 items resolved: #1 presence bitmask, #2 numpad, #3 lightning→events migration, #4 zero-wire season, #5 zero-trust rock perception, #6 unified a|verb|arg, #7 inventory r/S/T/a/E, #8 13 round-trip vectors locked, #9 FLEEING IN for Phase 15.]* **MANDATORY gate between discuss-phase and plan-phase.** This CONTEXT.md locks the shape; the grammar must be formally reviewed and committed as `15-SCHEMA.md` before PLAN.md is written. Every item below requires user input — planner pauses for confirmation on each before finalising:
...
> **⚠ Historical draft. Superseded by `15-SCHEMA.md` §10 (13 locked round-trip test vectors).** Kept for audit trail only.
...
*Gate resolved 2026-04-20: `15-SCHEMA.md` committed. PLAN.md may now be written.*
