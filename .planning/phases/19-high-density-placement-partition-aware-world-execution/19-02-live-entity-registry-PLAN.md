...
must_haves:
  truths:
    ...
    - "register/unregister are O(1) (sparse-set: dense list + entityId→index map, swap-and-pop)."
    - "No tick-handler logic is changed in this plan — Plan 03 owns the iteration refactor; this plan only stands up the registry + lifecycle hooks."
  artifacts:
...
<objective>
Stand up `LiveEntityRegistry` — a sparse-set bean whose iteration order is deterministic and whose add/remove are O(1) — and wire its lifecycle hooks at every entity-creation and entity-death site. This plan is **infrastructure only**: it does NOT change tick-handler iteration. Plan 03 consumes the registry; the consumer wave depends on this one.
...
Purpose: SCALE-07 prerequisite. Tick handlers currently grid-scan O(65,536) cells to find entities. `LiveEntityRegistry.snapshot()` will replace that with O(N) entity iteration — but Plan 03 does that. This plan only ensures the data structure exists and stays correct under register/death/composite-collapse lifecycle events.
Output: `LiveEntityRegistry` bean + lifecycle hooks at three call sites + Wave 0 unit tests + zero behavioural change to existing tick handlers.
</objective>
 ...
 * and {@code TickBroadcaster.onTick}. Plan 03 consumes this bean; Plan 02
...
- The `EntityEntry` record carries only `entityId` + `position` — Plan 03 consumers do `worldGrid.getCell(entry.position()).occupant()` to get the live `Entity` object. Storing the `Entity` here would require updates on every `Cell.withOccupant()` call (energy decay, etc.) and increase coupling — keep the registry minimal.
...
<task type="auto">
  <name>Task 2: Wire lifecycle hooks at registration, death, and composite-collapse call sites</name>
  <files>src/main/java/com/paralife/websocket/WorldWebSocketHandler.java, src/main/java/com/paralife/engine/DeathFinalizer.java, src/main/java/com/paralife/engine/SimulationEngine.java</files>
  <read_first>
...
1. **Modify** `src/main/java/com/paralife/websocket/WorldWebSocketHandler.java`:
   ...
   (b) Add `LiveEntityRegistry liveEntityRegistry` parameter to the @Autowired constructor (line 118 onwards). Forward `null` from any alternate test constructors. Note: Plan 01 also adds a parameter (`EligibleCellIndex`) — coordinate at execute-time; both parameters land in the same constructor signature.
...
3. **Modify** `src/main/java/com/paralife/engine/SimulationEngine.java`:
   ...
   (c) Position-update hook for movement: in `ActionResolver` (which already exists, see CLAUDE.md tick pipeline @Order(20)), after a successful move that calls `worldGrid.setCell` or `worldGrid.setEntity` for a moving entity, add `liveEntityRegistry.updatePosition(entityId, newPos)`. **Re-read `ActionResolver.java`** to find the exact move-resolution method and add `LiveEntityRegistry` ctor injection + the `updatePosition` call there. (If `ActionResolver` is too far from this plan's scope, defer the position-update hook to Plan 03 — but document the gap explicitly in the SUMMARY. The registry's `updatePosition` is idempotent on missing ids and safe to call from any handler order.)
...
4. Run `./gradlew test` — full regression suite. No tick-handler logic changes in this plan; existing tests should remain green. Any failure indicates a hook landed in the wrong site or an idempotency assumption was violated.
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - `grep -c "BotRegistry.unregisterByEntity\|botRegistry\.unregisterByEntity" src/main/java/com/paralife/engine/BotRegistry.java` indicates `BotRegistry.java` itself was NOT modified to add LiveEntityRegistry coupling (the hook is in DeathFinalizer, per PATTERNS.md). Equivalent check: `grep -c "LiveEntityRegistry" src/main/java/com/paralife/engine/BotRegistry.java` == 0.
    - `./gradlew test` exits 0 (full regression — 166+ existing tests remain green; no observable behaviour change in this task)
  </acceptance_criteria>
  <done>LiveEntityRegistry receives register/unregister/updatePosition events at every entity lifecycle site (registration, particle death, bonded-pair death, composite-collapse, optionally move). DeathFinalizer is the death-cleanup choke point — BotRegistry is not coupled to LiveEntityRegistry. Full regression suite passes; Plan 03 can now consume `liveEntityRegistry.snapshot()` knowing the data structure stays in sync.</done>
</task>
...
- `grep -rn "liveEntityRegistry" src/main/java/com/paralife/` shows hooks at: WorldWebSocketHandler.handleRegister, DeathFinalizer.finalize*, SimulationEngine.collapseToMember (and possibly ActionResolver.resolveMove).
- BotRegistry.java has no reference to LiveEntityRegistry (per PATTERNS.md analog evidence).
</verification>

<success_criteria>
- LiveEntityRegistry is registered as a Spring bean and ready for Plan 03 to consume.
- All register/unregister hooks are wired at the correct DeathFinalizer + WS + composite-collapse sites — no double-hooks, no missed sites.
