...
3. **Golden-trace test scope: what frames to capture?**
   ...
   - **RESOLVED:** SHA-256 digest of all successfully-sent `sendMessage` payloads, captured via a test-only `FrameEmitListener` seam on `OutboundSender` (listener fires inside `synchronized(session)` immediately after `session.sendMessage` returns). The capture is a **per-session digest map** — `Map<String, MessageDigest>` keyed by `sessionId`, each frame from session S hashed into `digests[S]` in send order (single drain VT per session ⇒ intra-session FIFO is the only ordering guarantee; cross-session emit order is intentionally NOT compared). Plan 03 captures the digest map against the post-Plan-02 / pre-Plan-04 codebase and pins it as a `private static final Map<String, String> EXPECTED_DIGESTS` constant in `GoldenTraceEquivalenceTest`; the test asserts both `hashMapA == hashMapB` (intra-run determinism) AND `hashMapA == EXPECTED_DIGESTS` (cross-plan equivalence) so Plan 04 cannot silently agree with itself. Metric-counter equivalence is deferred to the Phase 21 benchmark gate.

     **Per-session map (not single global digest) — rationale (REVIEWS H4 / Codex H / OpenCode H):** virtual-thread scheduling jitter on the OutboundSender drain VTs means cross-session emit order across two JVM runs is non-deterministic, even at the same seed. Within a single session, the drain VT is single-threaded over a FIFO bounded queue, so per-session order IS deterministic. A single global digest would conflate (deterministic) per-session content with (non-deterministic) cross-session interleaving and produce flaky baselines; per-session digests isolate the deterministic portion.
