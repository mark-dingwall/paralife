...
must_haves:
  truths:
    ...
    - "GoldenTraceEquivalenceTest passes — outbound frame bytes byte-identical vs the post-Plan-03 baseline."
  ...
  artifacts:
    - path: src/main/java/com/paralife/engine/SimulationEngine.java
      provides: "9 grid-scan sites refactored to LiveEntityRegistry.snapshot() iteration; nutrient-spawn pass (line 1172) preserved; processOvercrowding neighbour-count walk per entity preserved verbatim."
...
The `GoldenTraceEquivalenceTest` from Plan 03 is the merge gate: this plan's diff is correct iff that test stays green AND the full regression suite stays green.
...
**Position lookup via WorldGrid:** `EntityEntry.position()` carries the position recorded at registration. If an entity moved (ActionResolver did a `setEntity(newPos) + clearEntity(oldPos)`), the registry's position may be stale. Plan 02 wires `liveEntityRegistry.updatePosition` at the move site — but if that hook was deferred, refactored handlers must read the current position via `worldGrid.getCell(...)` lookup keyed on entityId, not from `entry.position()`. **Re-read ActionResolver.java to confirm `liveEntityRegistry.updatePosition` is wired before this plan starts.** If not, fix Plan 02 first; do not paper over.
...
<task type="auto" tdd="true">
  ...
  <read_first>
    ...
    - .planning/phases/19-high-density-placement-partition-aware-world-execution/19-PATTERNS.md (lines 192–230 — 9 grid-scan table; lines 207–225 — replace-pattern with shuffle invariant)
    ...
    - src/test/java/com/paralife/engine/GoldenTraceEquivalenceTest.java (the oracle from Plan 03)
  </read_first>
  <action>
...
2. **Refactor each of the 8 in-scope grid scans** (the 9th, processNutrientSpawning at line 1172, stays). For each site below, replace the `for (int x ...) for (int y ...)` block that materialises a Position list with the entity-list iteration pattern shown in the `<interfaces>` block above. Preserve the entity-type filter (`instanceof Particle`, `instanceof BondedPair`, `instanceof CompositeMember`) — the filter on `worldGrid.getCell(pos).occupant()` is the canonical source of truth for what's currently on that cell.

   Sites (line numbers from PATTERNS.md — re-verify before patching; line numbers may shift after Plan 02 lands):

   | Line | Method | Filter |
   |------|--------|--------|
   | 295–302 | processInteractions | `instanceof Particle` |
   | 421–428 | processInteractions | `instanceof CompositeMember` |
   | 505–512 | processInteractions | `instanceof BondedPair` |
   | 735–803 | processEnergyDecay | mixed — Particle and BondedPair branches; preserve branch logic |
   | 870–899 | processOvercrowding | `instanceof Particle \|\| instanceof BondedPair` |
   | 909–919 | processDeaths Phase 3a | particle/bonded; `if (!isAlive())` predicate preserved |
   | 924+ | processDeaths composite half | `instanceof CompositeMember` |
   | (one more — the 9th cited at A2) | (re-grep; verify) | per-method filter |
...
4. After all 8 sites are refactored, run the gate command. The full suite + GoldenTraceEquivalenceTest must be green.
  ...
  <acceptance_criteria>
    - `grep -c "liveEntityRegistry.snapshot" src/main/java/com/paralife/engine/SimulationEngine.java` >= 7 (one per refactored site; the 9th nutrient-spawn site stays grid-walk)
    ...
    - `./gradlew test --tests "com.paralife.engine.GoldenTraceEquivalenceTest"` exits 0 (D-10 gate)
    - `./gradlew test` exits 0 (full regression — 166+ tests + 4 new Wave 0 tests all green)
  </acceptance_criteria>
  <done>SimulationEngine's 8 in-scope grid scans now consume LiveEntityRegistry.snapshot(); nutrient-spawn pass preserved; @Order(10) unchanged; Collections.shuffle invariants preserved; GoldenTraceEquivalenceTest stays green.</done>
</task>
...
4. Run the gate command. The full suite + GoldenTraceEquivalenceTest must be green.
  ...
  <done>EnvironmentEngine per-entity segments and TickBroadcaster.onTick @Order(50) consume LiveEntityRegistry.snapshot(); diffusion / lightning / fertility passes preserved as grid-walks; @Order chain unchanged; STALLED-skip and outboundSender.offer paths verbatim; D-08/D-11 invariants preserved; GoldenTraceEquivalenceTest green; full regression green.</done>
</task>
...
|-----------|----------|-----------|-------------|-----------------|
| T-19-10 | Tampering | Refactored handler emits non-equivalent output | mitigate | GoldenTraceEquivalenceTest from Plan 03 is the regression gate; failure blocks merge. |
...
<verification>
- `./gradlew test --tests "com.paralife.engine.GoldenTraceEquivalenceTest"` — D-10 gate green.
...
- Current simulation semantics stable at existing milestone workloads — proven byte-for-byte by GoldenTraceEquivalenceTest.
