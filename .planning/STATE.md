...
| tech-debt | TD-22-C — `PopulationDynamicsTest.allThreeTypesSurvive500Ticks` `@Disabled`; probabilistic flat-line. ~~Pin RNG seed or widen tolerance — backlog Phase 999.x.~~ **RESOLVED 2026-06-23 (commit `c1d8db6`): gate deleted, not seed-pinned/widened.** Per Phase 16 "Drift Correction" precedent (D-04 #2, commit `2ec1d1c`) — tests pin spec mechanics; emergent ecosystem survival is judged by a human against the M5 visualiser or offline parameter search, not gated in JUnit. Salvage audit confirmed zero unique coverage lost (RPS combat / death / reproduction / nutrient / starvation / composite-formation all pinned deterministically elsewhere). Init-time `fertilityPatchesExistAfterInit` retained. | resolved | 2026-06-23 |
