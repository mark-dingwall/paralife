...
| tech-debt | TD-22-A — `MetabolismIntegrationTest.allTypesSurviveWithMetabolism` `@Disabled`; WorldGrid read-lock starvation under tick-loop write pressure. ~~Re-enable + pass under P21 benchmark conditions.~~ **RESOLVED 2026-06-23 (commit `65a9745` re-enable): root cause was carrier-thread starvation from the shared-JVM leaks closed in `876c8b1`, not a lock-fairness defect. Validated: tier-1 isolation 10/10 clean; tier-2 full suite `forkEvery=0` green (999 tests, 0 fail, 0 VT-exit warns, 0 XML errors) with the test executing (not skipped).** Cleared ahead of P21. _(Review note 2026-06-24: `allTypesSurviveWithMetabolism` is itself an emergent 30-bot/600-tick survival gate — the same class as the deleted TD-22-C — retained because it now passes stably as the metabolism-specific integration coverage; the line vs TD-22-C is pragmatic, not principled. Primary long-run emergent gate remains `EmergenceStabilityLoadTest`. Optional follow-up: RNG-seed it or hold it to the same standard as TD-22-C.)_ | resolved | 2026-06-23 |
