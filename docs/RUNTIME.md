...
architectural property (docs/ARCHITECTURE.md §Connection model; HARNESS.md §1 D-05/D-21);
...
HARNESS.md §1 D-02 ceiling) AND tuning under §2 has been exhausted.
...
ADMISSION.md §6 Backpressure for the queue-depth sizing math. **Lifecycle:** the value is read at
...
no public configuration surface. Wire format stays bit-exact (`SCHEMA.md`
LOCKED).
...
paralife:
  ...
  admission:
    backpressure:
      outbound-queue-size: 128    # default; tighten only with measured slow-client evidence per ADMISSION.md §6 Backpressure (new sessions only — see §2.2 lifecycle note)
```
