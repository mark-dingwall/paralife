...
| H12 | WHEN writing a report THE SYSTEM SHALL serialize snake_case and publish via atomic temp-rename (no torn or `.tmp` file observable). | §6 | `ReportWriterTest.writeOverwrite_fieldNamesAreSnakeCase` (camelCase absent); `writeOverwrite_tmpFileDoesNotExistAfterWrite` — `assertThat(tmpFile).doesNotExist()`; concurrency `concurrentWriteOverwrite_neverProducesTornFile` (50 VTs, final parses valid). |
